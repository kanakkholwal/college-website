...
export default function Toggle({ currentStatus,roomNumber, updateStatus }: {
    currentStatus: "available" | "occupied",
    roomNumber: string,
    updateStatus: (roomNumber:string,status: "available" | "occupied") => Promise<boolean>
}) {
    const [loading, setLoading] = useState<boolean>(false)

    return (
        <div className="flex items-center space-x-2">
            <Switch id="currentStatus"
                disabled={loading}
                checked={currentStatus === "available" ? true : false}
                onCheckedChange={(checked) => {
                    setLoading(true)
                    toast.promise(updateStatus(roomNumber ,(checked ? "available" : "occupied")), {
